...
function renderPhotos(lang){
  ...
  const varietyGrid = document.getElementById('photos-grid-variety');
  if (!venomGrid && !safeGrid && !varietyGrid) return;
  ...
  if (varietyGrid){
    varietyGrid.innerHTML = photosData.filter(p => p.type === 'variety').map(tileHtml).join('');
  }
}
